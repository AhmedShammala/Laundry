...
class PickupController: UIViewController,UICollectionViewDataSource ,UICollectionViewDelegateFlowLayout {
    ...
    override func viewWillAppear(_ animated: Bool) {
        self.MyStepView.Apply(title: "Next", Page: 2)
        self.MyStepView.NextBtn.addTarget(self, action: #selector(self.NextPage), for: .touchUpInside)
    }
    func NextPage()
    {
        let vc = self.storyboard?.instantiateViewController(withIdentifier: "DeliveryController") as? DeliveryController
